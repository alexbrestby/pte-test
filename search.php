...
if (mysqli_num_rows($result)>0 && (mysqli_num_rows($result) != $row_lenght[0])) {
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		if ($row["q_img"]) echo '<img src=assets/img/images/' . $row["q_img"] . ' alt="фото из вопроса">';
		echo '<br>';
		...
		if ($row["ans_1_img"] != null) echo '<img src=assets/img/images/' . $row["ans_1_img"] . ' alt="фото из ответа">';


		if ($row["ans_2_text"] != null) echo '<hr>- ' . $row["ans_2_text"];
		if ($row["ans_2_img"] != null) echo '<hr><img src=assets/img/images/' . $row["ans_2_img"] . ' alt="фото из ответа">';


		if ($row["ans_3_text"] != null) echo '<hr>- ' . $row["ans_3_text"];
		if ($row["ans_3_img"] != null) echo '<hr><img src=assets/img/images/' . $row["ans_3_img"] . ' alt="фото из ответа">';


		if ($row["ans_4_text"] != null) echo '<hr>- ' . $row["ans_4_text"];
		if ($row["ans_4_img"] != null) echo '<hr><img src=assets/img/images/' . $row["ans_4_img"] . ' alt="фото из ответа">';


		if ($row["ans_5_text"] != null) echo '<hr>- ' . $row["ans_5_text"] . '<br>';
		if ($row["ans_5_img"] != null) echo '<hr><img src=assets/img/images/' . $row["ans_5_img"] . ' alt="фото из ответа">';
